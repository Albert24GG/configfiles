...
local function factory(args)
    ...
    if not fs.is_dir(pspath) then
        naughty.notify { text = "bat-widget: invalid power supply path", timeout = 0 }
        return
    end
    ...
    function bat.get_batteries()
        line_callback("ls -1 " .. pspath, function(line)
    ...
    function bat.update()
        ...
        for i, battery in ipairs(batteries) do
            ...
            if tonumber(present) == 1 then
                ...
                local charge_full  = tonumber(first_line(bstr .. "/charge_full"))
                local charge_design = tonumber(first_line(bstr .. "/charge_full_design"))
    ...
    local bat_timer = gears.timer.start_new(timeout, function() 
        bat.update()
        return true
    end)
