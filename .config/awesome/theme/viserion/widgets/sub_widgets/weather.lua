...
local json = require(require("configuration.user-variables").ThemePath .. "widgets.sub_widgets.utils.json")
local naughty = require("naughty")
local wibox = require("wibox")
local gears = require("gears")
local beautiful = require("beautiful")

local CONGIF_DIR = gears.filesystem.get_configuration_dir()
...
local weather_popup = awful.popup {
    ...
    border_width = beautiful.weather_border_width,
...
local function worker(args)
    ...
    local api_key = read_first_line(args.api_key)
    ...
    local current_weather_widget = wibox.widget {
        ...
        update = function(self, weather)
            self:get_children_by_id('icon')[1]:set_image(CONGIF_DIR .. '/icons/' .. icon_pack_name .. '/' .. icon_map[weather.weather[1].icon] .. icons_extension)
            ...
            for i, day in ipairs(forecast) do
                ...
                local day_forecast = wibox.widget {
                    ...
                    },
                    {
                        {
                            {
                                image = CONGIF_DIR .. '/icons/' .. icon_pack_name .. '/' .. icon_map[day.weather[1].icon] .. icons_extension,
                                resize = true,
                                forced_width = 48,
                                forced_height = 48,
                                widget = wibox.widget.imagebox
    ...
    local hourly_forecast_graph = wibox.widget {
        step_width = 12,
        color = '#EBCB8B',
        background_color = beautiful.weather_bg,
        forced_height = 100,
        forced_width = 300,
    ...
    local function update_widget(widget, stdout, stderr)
        ...
        widget:set_image(CONGIF_DIR .. '/icons/' .. icon_pack_name .. '/' .. icon_map[result.current.weather[1].icon] .. icons_extension)
    ...
    weather_widget:buttons(awful.util.table.join(awful.button({}, 1, function()
        if weather_popup.visible then
            ...
            -- weather_popup:move_next_to(weather_widget)
            weather_popup:move_next_to(mouse.current_widget_geometry)
        end
    end)))
    ...
    watch(
        ...
        600,  -- API limit is 1k req/day; day has 1440 min; every 2 min is good
