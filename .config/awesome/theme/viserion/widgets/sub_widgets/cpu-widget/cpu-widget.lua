...
local cpu_usage = require(require("configuration.user-variables").ThemePath .. ".widgets.sub_widgets.cpu-widget.cpu-usage")
...
local function create_kill_process_button()
    return wibox.widget{
        {
            id = "icon",
            markup = "<span foreground='white'></span>",
            font = beautiful.icon_fontname .. 10,
            opacity = 0.1,
            widget = wibox.widget.textbox,
        },
...
local function worker(user_args)
    ...
    local spc = args.spc or 3
    local mar = args.mar or 0
    local icfsize = args.icfsize or 20
    local icon = args.icon or "$$"
    local settings = args.settings or function() end

    local cpuload_widget = wibox.widget{
        ...
        margins = mar,
        {
...

            {
                ...
                font = beautiful.icon_fontname .. icfsize
            },
