...
# paru constants
set -l hasopt '__fish_contains_opt -s G -s V -s P -s S -s D -s Q -s R -s U -s T -s F -s L -s C database query sync remove upgrade deptest files version repoctl chrootctl gendb'
...
complete -c $progname -n "$hasopt" -s b -l dbpath -d 'Alternate database location' -xa "(__fish_complete_directories)"
complete -c $progname -n "$hasopt" -s r -l root -d 'Alternate installation root' -xa "(__fish_complete_directories)"
complete -c $progname -n "$hasopt" -s v -l verbose -d 'Output more status messages' -f
complete -c $progname -n "$hasopt" -l arch -d 'Alternate architecture' -f
complete -c $progname -n "$hasopt" -l cachedir -d 'Alternate package cache location' -xa "(__fish_complete_directories)"
complete -c $progname -n "$hasopt" -l color -d 'Colorize the output' -fa '{auto,always,never}'
complete -c $progname -n "$hasopt" -l config -d 'Alternate config file' -r
complete -c $progname -n "$hasopt" -l confirm -d 'Always ask for confirmation' -f
complete -c $progname -n "$hasopt" -l debug -d 'Display debug messages' -f
complete -c $progname -n "$hasopt" -l disable-download-timeout -d 'Use relaxed timeouts for download' -f
complete -c $progname -n "$hasopt" -l gpgdir -d 'Alternate home directory for GnuPG' -xa "(__fish_complete_directories)"
complete -c $progname -n "$hasopt" -l hookdir -d 'Alternate hook location' -xa "(__fish_complete_directories)"
complete -c $progname -n "$hasopt" -l logfile -d 'Alternate log file'
complete -c $progname -n "$hasopt" -l noconfirm -d 'Bypass any confirmation' -f
complete -c $progname -n "$hasopt" -l sysroot -d 'Operate on a mounted guest system (root-only)' -xa "(__fish_complete_directories)"
...
# New options
complete -c $progname -n "$hasopt" -l repo -d 'Assume targets are from the repositories' -f
complete -c $progname -n "$hasopt" -s a -l aur -d 'Assume targets are from the AUR' -f
complete -c $progname -n "$hasopt" -l aururl -d 'Set an alternative AUR URL' -f
complete -c $progname -n "$hasopt" -l aurrpcurl -d 'Set an alternative URL for the AUR /rpc endpoint' -f
complete -c $progname -n "$hasopt" -l clonedir -d 'Directory used to download and run PKGBUILDs' -r
complete -c $progname -n "$hasopt" -l makepkg -d 'Makepkg command to use' -f
complete -c $progname -n "$hasopt" -l pacman -d 'Pacman command to use' -f
complete -c $progname -n "$hasopt" -l pacman-conf -d 'Pacman-conf command to use' -f
complete -c $progname -n "$hasopt" -l git -d 'Git command to use' -f
complete -c $progname -n "$hasopt" -l asp -d 'Asp command to use' -f
complete -c $progname -n "$hasopt" -l gpg -d 'Gpg command to use' -f
complete -c $progname -n "$hasopt" -l fm -d 'File manager to use' -f
complete -c $progname -n "$hasopt" -l pager -d 'Pager command to use' -f
complete -c $progname -n "$hasopt" -l completioninterval -d 'Refresh interval for completion cache' -f
complete -c $progname -n "$hasopt" -l sortby -d 'Sort AUR results by a specific field during search' -xa "{votes,popularity,id,baseid,name,base,submitted,modified}"
complete -c $progname -n "$hasopt" -l searchby -d 'Search for AUR packages by querying the specified field' -xa "{name,name-desc,maintainer,depends,checkdepends,makedepends,optdepends}"
complete -c $progname -n "$hasopt" -l limit -d 'Limit number of search results' -x
complete -c $progname -n "$hasopt" -l upgrademenu -d 'Show a detailed list of updates with the option to skip any' -f
complete -c $progname -n "$hasopt" -l noupgrademenu -d 'Do not show the upgrade menu' -f
complete -c $progname -n "$hasopt" -l removemake -d 'Remove make deps after install' -f
complete -c $progname -n "$hasopt" -l noremovemake -d 'Do not remove make deps after install' -f
complete -c $progname -n "$hasopt" -l topdown -d 'Shows repository packages first and then aur' -f
complete -c $progname -n "$hasopt" -l bottomup -d 'Shows aur packages first and then repository' -f
complete -c $progname -n "$hasopt" -l devel -d 'Check -git/-svn/-hg development version' -f
complete -c $progname -n "$hasopt" -l nodevel -d 'Disable development version checking' -f
complete -c $progname -n "$hasopt" -l develsuffixes -d 'Suffixes to define development packages' -f
complete -c $progname -n "$hasopt" -l cleanafter -d 'Clean package sources after successful build' -f
complete -c $progname -n "$hasopt" -l nocleanafter -d 'Disable package sources cleaning' -f
complete -c $progname -n "$hasopt" -l redownload -d 'Redownload PKGBUILD of package even if up-to-date' -f
complete -c $progname -n "$hasopt" -l noredownload -d 'Do not redownload up-to-date PKGBUILDs' -f
complete -c $progname -n "$hasopt" -l provides -d 'Look for matching providers when searching for packages' -f
complete -c $progname -n "$hasopt" -l noprovides -d 'Just look for packages by pkgname' -f
complete -c $progname -n "$hasopt" -l pgpfetch -d 'Prompt to import PGP keys from PKGBUILDs' -f
complete -c $progname -n "$hasopt" -l nopgpfetch -d 'Do not prompt to import PGP keys' -f
complete -c $progname -n "$hasopt" -l newsonupgrade -d 'Print new news during sysupgrade' -f
complete -c $progname -n "$hasopt" -l useask -d 'Automatically resolve conflicts using pacmans ask flag' -f
complete -c $progname -n "$hasopt" -l nouseask -d 'Confirm conflicts manually during the install' -f
complete -c $progname -n "$hasopt" -l savechanges -d 'Commit changes to pkgbuilds made during review' -f
complete -c $progname -n "$hasopt" -l nosavechanges -d "Don't commit changes to pkgbuilds made during review" -f
complete -c $progname -n "$hasopt" -l failfast -d 'Exit as soon as any AUR packages fail to build' -f
complete -c $progname -n "$hasopt" -l nofailfast -d "Don't Exit as soon as any AUR packages fail to build" -f
complete -c $progname -n "$hasopt" -l keepsrc -d "Keep src/ and pkg/ directories after building packages" -f
complete -c $progname -n "$hasopt" -l nokeepsrc -d "Don't keep src/ and pkg/ directories after building packages" -f
complete -c $progname -n "$hasopt" -l combinedupgrade -d 'Refresh then perform the repo and AUR upgrade together' -f
complete -c $progname -n "$hasopt" -l nocombinedupgrade -d 'Perform the repo upgrade and AUR upgrade separately' -f
complete -c $progname -n "$hasopt" -l batchinstall -d 'Build multiple AUR packages then install them together' -f
complete -c $progname -n "$hasopt" -l nobatchinstall -d 'Build and install each AUR package one by one' -f
complete -c $progname -n "$hasopt" -l rebuild -d 'Always build target packages' -f
complete -c $progname -n "$hasopt" -l norebuild -d 'Skip package build if in cache and up to date' -f
complete -c $progname -n "$hasopt" -l mflags -d 'Pass the following options to makepkg' -f
complete -c $progname -n "$hasopt" -l gitflags -d 'Pass the following options to git' -f
complete -c $progname -n "$hasopt" -l gpgflags -d 'Pass the following options to gpg' -f
complete -c $progname -n "$hasopt" -l sudoflags -d 'Pass the following options to sudo' -f
complete -c $progname -n "$hasopt" -l batflags -d 'Pass the following options to sudo' -f
complete -c $progname -n "$hasopt" -l fmflags -d 'Pass the following options to file manager' -f
complete -c $progname -n "$hasopt" -l chrootflags -d 'Pass the following options to makechrootpkg' -f
complete -c $progname -n "$hasopt" -l sudoloop -d 'Loop sudo calls in the background to avoid timeout' -f
complete -c $progname -n "$hasopt" -l nosudoloop -d 'Do not loop sudo calls in the background' -f
complete -c $progname -n "$hasopt" -l chroot -d 'Build packages in a chroot' -f
complete -c $progname -n "$hasopt" -l nochroot -d "Don't build packages in a chroot" -f
complete -c $progname -n "$hasopt" -l sign -d 'Sign packages with gpg' -f
complete -c $progname -n "$hasopt" -l nosign -d "Don't sign packages with gpg" -f
complete -c $progname -n "$hasopt" -l keeprepocache -d 'Keep old versions of packages with local repo' -f
complete -c $progname -n "$hasopt" -l nokeeprepocache -d "Don't keep old versions of packages with local repo" -f
complete -c $progname -n "$hasopt" -l signdb -d 'Sign databases with gpg' -f
complete -c $progname -n "$hasopt" -l nosigndb -d "Don't sign databases with gpg" -f
complete -c $progname -n "$hasopt" -l localrepo -d 'Build packages in a local repo' -f
complete -c $progname -n "$hasopt" -l nolocalrepo -d "Don't build packages in a local repo" -f
complete -c $progname -n "$hasopt" -l skipreview -d 'Skip the review process' -f
complete -c $progname -n "$hasopt" -l review -d "Don't skip the review process" -f
